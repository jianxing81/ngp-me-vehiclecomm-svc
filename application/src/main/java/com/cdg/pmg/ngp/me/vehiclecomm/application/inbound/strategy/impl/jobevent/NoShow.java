...
public class NoShow extends AbstractJobEvent {
  ...
  public void handleJobEvent(EsbJob esbJob, String ivdJobEventTopic) {
    ...
    var byteDataRepresentation =
        byteToBeanConverter.noShowRequestConverter(esbJob.getIvdMessageRequest().getMessage());
    ...
    // End the process if driver location is invalid
    if (isValidLocation(esbJob)) return;
    ...
    callVehicleSvcToUpdateVehicleState(esbJob, VehicleEvent.NO_SHOW);
  ...
  /**
   * Method to check if location is valid or not
   *
   * @param esbJob - esbJob
   * @return boolean
   */
  protected boolean isValidLocation(EsbJob esbJob) {
    if (!esbJob.getByteData().getIsValidLocation()) {
      log.info(VehicleCommAppConstant.INVALID_LOCATION);
      return true;
    }
    return false;
  }
