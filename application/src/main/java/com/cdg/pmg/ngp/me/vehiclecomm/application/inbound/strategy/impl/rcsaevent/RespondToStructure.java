 ...
 *
 * @see <a
 *     href="https://comfortdelgrotaxi.atlassian.net/wiki/spaces/NGP/pages/1288275443/4.2.1.15.29+Respond+to+Structure+Message+Event+Use+Case+Design+Document">4.2.1.15.29
 *     Respond to Structure Message Event Use Case Design Document</a>
 */
@Slf4j
@Component
@RequiredArgsConstructor
public class RespondToStructure extends AbstractRcsaEvent {
  ...
  public void handleRcsaEvent(Rcsa rcsaEvent, String rcsaEventTopic) {
    ...
    log.debug("handle Rcsa Event for Respond To Structure: ");
    // Convert byte data to a pojo
    var byteDataRepresentation =
        byteToBeanConverter.respondToStructureConverter(
    ...
    vehicleCommDomainService.setRcsaEventType(rcsaEvent, RcsaEvents.RESPOND_TO_STRUCTURE_MESSAGE);
    // publish message to ngp.me.rcsa.event
    produceMsgToRcsaEvent(rcsaEvent);
  }

  @Override
  public IvdMessageEnum ivdMessageEnum() {
    return IvdMessageEnum.RESPOND_TO_STRUCTURE_MESSAGE;
  }
}
