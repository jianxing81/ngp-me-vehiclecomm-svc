...
import com.cdg.pmg.ngp.me.vehiclecomm.techframework.messaging.mappers.IvdVehicleEventMapper;
...
@Component
public class EsbVehicleEventConsumer {

  private final KafkaMessageHandler kafkaMessageHandler;
  private final IvdVehicleEventMapper ivdVehicleEventMapper;
  private final RefreshableProperties refreshableProperties;

  /** Method to handle VehicleEvents from kafka */
  @Value("${spring.kafka.vehcomm.topic.ivd_vehicle_event}")
  private String ivdVehicleEventTopic;
  ...
  @KafkaListener(
      topics = "${spring.kafka.vehcomm.topic.ivd_vehicle_event}",
      groupId = "${spring.kafka.vehcomm.group}")
  public void handleEsbVehicleEvent(@Payload EsbVehicleEvent esbVehicleEvent) {
    var ivdInboundEvent =
        ivdVehicleEventMapper.esbVehicleEventToIvdInboundEvent(
            esbVehicleEvent, refreshableProperties);
    kafkaMessageHandler.esbVehicleEventHandler(ivdInboundEvent, ivdVehicleEventTopic);
  }
}
